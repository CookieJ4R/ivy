...
from model.domain.tag import Tag
from model.schema.tag_model import CreateTagRequest, TagResponse, TagUsageResponse, UpdateTagRequest
from util.helper import generate_random_hex_color
from sqlalchemy.exc import IntegrityError
from sqlalchemy import  select, delete, func, distinct

async def create_tag(tag: CreateTagRequest) -> int:
    """
    Creates a new tag.
    :param tag: Request containing tag name and color to create.
    """
    tag_name = tag.name.strip()
    if tag_name == "":
        raise ValueError("Tag name cannot be empty.")

    tag_color = tag.color.strip() or generate_random_hex_color()
...
async def update_tag(tag: UpdateTagRequest):
    """
    ...
    :param tag: The update request containing tag ID, name and color.
    ...
    async with AsyncSessionLocal() as session:
        ...
        if existing_tag is None:
            raise ValueError(f"Tag with ID '{tag.id}' does not exist.")
...
async def list_tags_usage() -> TagUsageResponse:
    """
    ...
    :return: TagUsageResponse containing usage statistics.
    """
    async with AsyncSessionLocal() as session:
        ...
        return TagUsageResponse(used=used_tags, unused=total_tags - used_tags)

async def list_tags() -> list[TagResponse]:
    """
    Lists all tags.
    :return: A list of TagResponse instances.
    """
    async with AsyncSessionLocal() as session:
        curr =  (await session.scalars(select(Tag))).all()
        return [TagResponse.model_validate(t) for t in curr]
