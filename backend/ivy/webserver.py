...
class Webserver:
    ...
    def __init__(self, db: DatabaseManager):
        ...
        self.dist_path = os.getenv("DIST_PATH", FRONTEND_DIST)
        self.file_storage_path = os.getenv("FILE_STORAGE_PATH", FILE_STORAGE_PATH)
        ...
        self.app.mount(
            "/data",
            StaticFiles(directory=self.file_storage_path),
            name="data",
        )
        ...
        if os.getenv("DEPLOYMENT_MODE") == "docker":
            self.app.mount(
                "/assets",
                StaticFiles(directory=os.path.join(self.dist_path, "assets")),
                name="assets",
            )
            async def serve_spa(full_path: str):
                file = os.path.join(self.dist_path, full_path)
                if os.path.isfile(file):
                    return FileResponse(file)
                return FileResponse(os.path.join(self.dist_path, "index.html"))
